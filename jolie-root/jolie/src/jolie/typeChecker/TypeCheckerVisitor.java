...
public class TypeCheckerVisitor implements OLVisitor {
    private static List<String> enforceTypeChecking = new ArrayList<>();
    ...
    static List<String> getEnforceTypeChecking() {
        return enforceTypeChecking;
    }
    ...
    private void processSimilarAssignments(VariablePathNode variablePath, OLSyntaxNode expression) {
        ...
        if (TermType.isMeaningful(expressionTerm.type())) {
            String assert_hastype = "(assert (hasType " + variablePathTerm.id() + " " + expressionTerm.type().id() + "))\n";
            formula += assert_hastype;
            if (enforceTypeChecking.isEmpty()) {
                enforceTypeChecking.add(assert_hastype);
            } else {
                for (int i = 0; i < enforceTypeChecking.size(); i++) {
                    if (enforceTypeChecking.get(i).compareTo(assert_hastype) != 0) {
                        enforceTypeChecking.add(assert_hastype);
                    }
                }
            }
